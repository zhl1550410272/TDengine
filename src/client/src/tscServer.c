...
void *tscProcessMsgFromServer(char *msg, void *ahandle, void *thandle) {
  ...
  if (msg == NULL) {
    ...
    if (rspCode == TSDB_CODE_REDIRECT) {
      ...
      /*
       * not_active_table: 1. the virtual node may fail to create table, since the procedure of create table is
       * asynchronized, the virtual node may have not create table till now, so try again by using the new metermeta.
...
int32_t tscLaunchJoinSubquery(SSqlObj *pSql, int16_t tableIndex, SJoinSubquerySupporter *pSupporter) {
  ...
  if (QUERY_IS_JOIN_QUERY(pQueryInfo->type)) {
    ...
    tscTrace(
        ...
        pNewQueryInfo->colList.numOfCols, pNewQueryInfo->fieldsInfo.numOfOutputCols,
        pNewQueryInfo->pMeterInfo[0]->name);
...
//    tscFreeSqlObj(tres);
  ...
  tscTrace("%p Async insertion completed, destroy data block list", pParentObj);
...
void tscRetrieveFromVnodeCallBack(void *param, TAOS_RES *tres, int numOfRows) {
  ...
  if (numOfRows > 0) {
    ...
    if (num > tsMaxNumOfOrderedResults && tscIsProjectionQueryOnSTable(pQueryInfo, 0)) {
      tscError("%p sub:%p num of OrderedRes is too many, max allowed:%" PRId64 " , current:%" PRId64, pPObj, pSql,
               tsMaxNumOfOrderedResults, num);
    ...
    // keep this value local variable, since the pState variable may be released by other threads, if atomic_add
    // opertion increases the finished value up to pState->numOfTotal value, which means all subqueries are completed.
    ...
    tscCreateLocalReducer(trsupport->pExtMemBuffer, pState->numOfTotal, pDesc, trsupport->pFinalColModel, &pPObj->cmd,
                          &pPObj->res);
...
void tscRetrieveDataRes(void *param, TAOS_RES *tres, int code) {
  ...
  if (pState->code != TSDB_CODE_SUCCESS) {  // failed, abort
    if (vnodeInfo != NULL) {
      ...
      tscTrace("%p sub:%p query failed,orderOfSub:%d,global code:%d", pParentSql, pSql, trsupport->subqueryIndex,
               pState->code);
    }
...
int tscProcessMeterMetaRsp(SSqlObj *pSql) {
  ...
  pMeterMetaInfo->pMeterMeta = (SMeterMeta *)taosCachePut(tscCacheHandle, pMeterMetaInfo->name, (char *)pMeta,
...
int tscProcessMultiMeterMetaRsp(SSqlObj *pSql) {
  ...
  for (i = 0; i < totalNum; i++) {
    ...
    pMeta->index = 0;
    (void)taosCachePut(tscCacheHandle, pMultiMeta->meterId, (char *)pMeta, size, tsMeterMetaKeepTimer);
  }
...
int tscProcessMetricMetaRsp(SSqlObj *pSql) {
  ...
  for (int32_t i = 0; i < num; ++i) {
    ...
    pMeterMetaInfo->pMetricMeta = (SMetricMeta *)taosCachePut(tscCacheHandle, name, (char *)metricMetaList[i],
...
int tscProcessShowRsp(SSqlObj *pSql) {
  ...
  pMeterMetaInfo->pMeterMeta =
      (SMeterMeta *)taosCachePut(tscCacheHandle, key, (char *)pMeta, size, tsMeterMetaKeepTimer);
  ...
  for (int16_t i = 0; i < pMeta->numOfColumns; ++i) {
    ...
    pQueryInfo->fieldsInfo.pSqlExpr[i] = tscSqlExprInsert(
        pQueryInfo, i, TSDB_FUNC_TS_DUMMY, &index, pMeterSchema[i].type, pMeterSchema[i].bytes, pMeterSchema[i].bytes);
  }
...
int tscProcessDropTableRsp(SSqlObj *pSql) {
  ...
  SMeterMeta *pMeterMeta = taosCacheAcquireByName(tscCacheHandle, pMeterMetaInfo->name);
  if (pMeterMeta == NULL) {
    /* not in cache, abort */
    return 0;
  }
...
int tscGetMeterMeta(SSqlObj *pSql, SMeterMetaInfo *pMeterMetaInfo) {
  ...
  pMeterMetaInfo->pMeterMeta = (SMeterMeta *)taosCacheAcquireByName(tscCacheHandle, pMeterMetaInfo->name);
  if (pMeterMetaInfo->pMeterMeta != NULL) {
    SMeterMeta *pMeterMeta = pMeterMetaInfo->pMeterMeta;
...
int tscGetMetricMeta(SSqlObj *pSql, int32_t clauseIndex) {
  ...
  for (int32_t i = 0; i < pQueryInfo->numOfTables; ++i) {
    ...
    SMetricMeta *ppMeta = (SMetricMeta *)taosCacheAcquireByName(tscCacheHandle, tagstr);
    if (ppMeta == NULL) {
      required = true;
      break;
    } else {
  ...
  for (int32_t i = 0; i < pQueryInfo->numOfTables; ++i) {
    ...
    SMeterMeta *pMeterMeta = taosCacheAcquireByName(tscCacheHandle, pMMInfo->name);
...
//  }
  ...
  //
  //        taosCacheRelease(tscCacheHandle, (void **)&(pMeterMetaInfo->pMetricMeta), false);
  //        pMeterMetaInfo->pMetricMeta = (SMetricMeta *)taosCacheAcquireByName(tscCacheHandle, tagstr);
  //      }
  //    }
  //
  //    tscFreeSqlObj(pNew);
